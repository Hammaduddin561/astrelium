...
import { AstreliumVsCompetitorsProvider } from './components/AstreliumVsCompetitors';

const execAsync = promisify(exec);

export function activate(context: vscode.ExtensionContext) {
    const provider = new ChatViewProvider(context.extensionUri);
    const comparisonProvider = new AstreliumVsCompetitorsProvider(context.extensionUri);
    
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider(ChatViewProvider.viewType, provider),
        vscode.window.registerWebviewViewProvider(AstreliumVsCompetitorsProvider.viewType, comparisonProvider)
    );
